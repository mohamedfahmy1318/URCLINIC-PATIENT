...
class _AirtelMoneyDialogState extends State<AirtelMoneyDialog> {
  ...
  Future<void> _handleClick() async {
    final String transactionId = "${const Uuid().v1()}-${widget.bookingId}";
...
Future<AirtelAuthModel> authorizeAirtelClient() async {
  final Map<dynamic, dynamic> request = {"client_id": appConfigs.value.airtelMoney.airtelClientid, "client_secret": appConfigs.value.airtelMoney.airtelSecretkey, "grant_type": "client_credentials"};
...
Future<bool> checkAirtelPaymentStatus(
  ...
  await authorizeAirtelClient().then((value) async {
    ...
    res = AirtelPaymentResponse.fromJson(
      await handleResponse(await airtelPayBuildHttpResponse('standard/v1/payments/$txnId', extraKeys: {'X-Country': airtel_country_code, 'X-Currency': airtel_currency_code, 'access_token': '${value.accessToken}', 'isAirtelMoney': true})),
    );
...
Future<Response> airtelPayBuildHttpResponse(
  ...
  if (await isNetworkAvailable()) {
    final headers = buildHeaderForAirtelMoney(extraKeys!['access_token'], extraKeys['X-Country'], extraKeys['X-Currency']);
